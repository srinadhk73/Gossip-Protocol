defmodule PushSumActors do

    def createActors(noOfNodes, topology, algorithm) do
        if topology == "full" do
                #NList = Enum.to_list 1..noOfNodes
                neighbor = Enum.to_list 1..noOfNodes
                for i <- 1..noOfNodes do
                    neighbor = List.delete_at(neighbor, i)
                    pid = spawn(fn -> PushSum.start_link(i,neighbor) end)
                    Process.monitor(pid)
                end
              General.pushfunc(noOfNodes)
            end
